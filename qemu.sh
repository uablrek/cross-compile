...
cmd_env() {
	...
	test -n "$long_opts" && export $long_opts # (before admin)
	eset admin=$dir/admin.sh
	if test -x $admin; then
		if grep -qF 'cmd_qemu_setup()' $admin; then
			eval $($admin qemu_setup)
		else
			log "WARNING: No qemu_setup in [$admin]"
		fi
	fi
